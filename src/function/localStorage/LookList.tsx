...
export const lookListCreate = (listSelect:boolean[]) => {
    ...
    if(!currentList){
        newList = [{id: 1, title: lookBoxText[listSelect.findIndex(x=>x)].title}];
    }else{
        newList = Object.assign([...currentList, {id: !currentList ? 1 :currentList.length+1, title: lookBoxText[listSelect.findIndex(x=>x)].title}]);
    }
